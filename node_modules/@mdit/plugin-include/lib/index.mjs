...
`):e},E=[/^\/\/ ?#?((?:end)?region) ([\w*-]+)$/,/^\/\* ?#((?:end)?region) ([\w*-]+) ?\*\/$/,/^#pragma ((?:end)?region) ([\w*-]+)$/,/^<!-- #?((?:end)?region) ([\w*-]+) -->$/,/^#((?:End )Region) ([\w*-]+)$/,/^::#((?:end)region) ([\w*-]+)$/,/^# ?((?:end)?region) ([\w*-]+)$/],S=/^@include\(([^)]+(?:\.[a-z0-9]+))(?:#([\w-]+))?(?:\{(\d+)?-(\d+)?\})?\)$/,v=(e,r,l,o=!1)=>{const[t,n,d]=r.exec(e.trim())||[];return Boolean(t&&n&&d===l&&n.match(o?/^[Ee]nd ?[rR]egion$/:/^[rR]egion$/))},F=(e,r)=>{let l=null,o=-1;for(const[t,n]of e.entries())if(l===null){for(const d of E)if(v(n,d,r)){o=t+1,l=d;break}}else if(v(n,l,r,!0))return{lineStart:o,lineEnd:t};return null},$=(e,{cwd:r,includedFiles:l,resolvedPath:o})=>{const{filePath:t}=e;let n=t;if(!p.isAbsolute(t)){if(!r)return console.error(`[@mdit/plugin-include]: Error when resolving path: ${t}`),`
Error when resolving path
`;n=p.resolve(r,t)}if(l.push(n),!P.existsSync(n))return console.error(`[@mdit/plugin-include]: ${n} not found`),`
File not found
`;const d=P.readFileSync(n).toString().replace(w,`
`).split(`
