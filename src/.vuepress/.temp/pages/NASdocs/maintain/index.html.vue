...
<li class="task-list-item"><input type="checkbox" class="task-list-item-checkbox" id="task-item-0" checked="checked" disabled="disabled"><label class="task-list-item-label" for="task-item-0"> 实验室数据快照</label></li>
<li class="task-list-item"><input type="checkbox" class="task-list-item-checkbox" id="task-item-1" checked="checked" disabled="disabled"><label class="task-list-item-label" for="task-item-1"> public权限设置全公开</label></li>
<li class="task-list-item"><input type="checkbox" class="task-list-item-checkbox" id="task-item-2" disabled="disabled"><label class="task-list-item-label" for="task-item-2"> private挂载进入GPU容器内</label></li>
<li class="task-list-item"><input type="checkbox" class="task-list-item-checkbox" id="task-item-3" checked="checked" disabled="disabled"><label class="task-list-item-label" for="task-item-3"> 新建一个数据集，用来保存</label></li>
<li class="task-list-item"><input type="checkbox" class="task-list-item-checkbox" id="task-item-4" disabled="disabled"><label class="task-list-item-label" for="task-item-4"> 服务器加内存，梭哈是一种智慧</label></li>
<li class="task-list-item"><input type="checkbox" class="task-list-item-checkbox" id="task-item-5" checked="checked" disabled="disabled"><label class="task-list-item-label" for="task-item-5"> 寻找一种更优美的方式桥接网络</label></li>
</ul>
</div>
...
<pre><code>ZFS文件系统，存储池、vdev、数据集、硬盘之间关系。
</code></pre>
</div>
...
<img src="@source/NASdocs/maintain/img/rep1.png" alt="rep1.jpg" loading="lazy"></li>
<li>选择<code v-pre>更换</code>之后，可以选择任意空闲硬盘，<strong>替换后的硬盘需要比原先硬盘大才行</strong>；之后需要等待半天的数据导入，就完成了替换
<img src="@source/NASdocs/maintain/img/rep2.png" alt="rep2.jpg" loading="lazy"></li>
</ul>
<h3 id="_2-硬盘损坏-掉盘" tabindex="-1"><a class="header-anchor" href="#_2-硬盘损坏-掉盘" aria-hidden="true">#</a> 2. 硬盘损坏/掉盘</h3>
<div class="hint-container danger">
<p class="hint-container-title">高危故障</p>
<pre><code>硬盘坏盘是最常见的故障，也是最危险的故障。
...
<p><strong>ONLINE为正常状态，在线状态；
DEGRADED为有硬盘损坏的状态，降级状态；</strong></p>
</div>
<p>损坏可以分为一下几种</p>
...
<img src="@source/NASdocs/maintain/img/break.png" alt="break.jpg" loading="lazy"></li>
...
<img src="@source/NASdocs/maintain/img/replace.png" alt="replace.jpg" loading="lazy"></li>
</ul>
...
<p>目前的NAS为6块硬盘组成一个<code v-pre>vdev</code>的存储池，可以添加新的<code v-pre>vdev</code>，使得原先的存储池变成两个<code v-pre>vdev</code>，从而在路径不变的情况下提升原先文件夹的容量。
<strong>新加vdev的raid方式需要和原来的vdev相同，不然无法扩展。原先的vdev为6块盘的raid-z2，因此建议新加的盘也组成raid-z2，加6块盘，刚好服务器最多支持12块盘。</strong></p>
</div>
...
<img src="@source/NASdocs/maintain/img/addvdev.png" alt="addvdev.jpg" loading="lazy"></li>
</ul>
...
<ul>
<li>
<p>首先在原先服务器上断开(导出)存储池</p>
<figure><img src="@source/NASdocs/maintain/img/discon.png" alt="断开存储池" tabindex="0" loading="lazy"><figcaption>断开存储池</figcaption></figure>
</li>
<li>
<p>在全部硬盘插入新的服务器后，在新的服务器上选择导入存储池即可。
<img src="@source/NASdocs/maintain/img/import.png" alt="import.jpg" loading="lazy"></p>
</li>
<li>
...
<p>没有那个金刚钻就不要揽瓷器活......
这个操作不做介绍，需要有足够的能力；否则秘钥丢失，数据火葬场。</p>
</div>
<div class="hint-container note">
<p class="hint-container-title">现状说明</p>
<p>实验室目前的NAS没有加密，今后也不打算加密</p>
</div>
<h3 id="_6-邮件提醒" tabindex="-1"><a class="header-anchor" href="#_6-邮件提醒" aria-hidden="true">#</a> 6. 邮件提醒</h3>
<div class="hint-container note">
<p class="hint-container-title">现状说明</p>
<p>目前没有设置邮件提醒，比较复杂，后续设置人员设置完成后记得补充文档</p>
</div>
...
<div class="hint-container note">
<p class="hint-container-title">现状说明</p>
<p>目前主要有两个数据集<code v-pre>public</code>和<code v-pre>private</code>数据集，分别用来保存私密文件和公开文件。两个数据集均开启了<code v-pre>去重</code>与<code v-pre>压缩</code>，提高硬盘使用效率。因为大家的数据集重复率很高。</p>
<p>新建<code v-pre>resource</code>数据集，用于存放个钟公开数据集与其他资源文件，打算做成只读。</p>
</div>
...
<div class="hint-container danger">
<p class="hint-container-title">高危操作</p>
<p>错误的权限设置虽然不会让数据无法读取（因为root永远最大），但是会破坏原来的文件私有性或者公有性。不建议随意更改。</p>
</div>
<div class="hint-container warning">
<p class="hint-container-title">现状（bug）说明</p>
<p><code v-pre>private</code>的权限为ACL，但是似乎和普通的通常权限没有什么区别，因为目前<code v-pre>私有</code>的控制是由<code v-pre>主目录</code>中的权限只有所属人有权限来控制（参考下一章“用户管理”）。  能跑就不要动，按理来说改回UOG权限也没问题，但是不要选择递归，这样会覆盖下面的<code v-pre>主目录</code>设置的权限，破坏私有性。</p>
</div>
<div class="hint-container warning">
<p class="hint-container-title">现状（bug）说明</p>
<p>现状(bug)说明
<code v-pre>public</code>权限有问题，没办法让新建文件夹有读写权限</p>
<ul>
<li>现在服务器有两个数据集<code v-pre>public</code>和<code v-pre>private</code>。
<code v-pre>public</code>的所属人是master，所属组是<code v-pre>lab-student</code>。正是这样，组内的学生才对其有全部读写执行权限。
<img src="@source/NASdocs/maintain/img/public.png" alt="public.jpg" loading="lazy"></li>
</ul>
</div>
<div class="hint-container warning">
<p class="hint-container-title">注意</p>
<p>权限目前有点乱，后面再改吧，配合nextcloud容器来控制</p>
</div>
<h2 id="三、用户管理" tabindex="-1"><a class="header-anchor" href="#三、用户管理" aria-hidden="true">#</a> 三、用户管理</h2>
<div class="hint-container tip">
<p class="hint-container-title">前置知识</p>
<pre><code>linux的用户、组、所有人、所属组概念（即）
</code></pre>
</div>
...
<div class="hint-container warning">
<p class="hint-container-title">现状（bug）说明</p>
<pre><code>系统目前同时拥有一个管理员用户`master`，属于`admin`组（本来想叫master组，但是起错名字了），不过目前没有管理员权限，，，，没设置好，还是用root敲命令行吧
</code></pre>
</div>
...
<img src="@source/NASdocs/maintain/img/dir.png" alt="dir.jpg" loading="lazy"></li>
<li>权限设置，只有用户本身有读写执行权限，其他人没有任何权限，这样能保证私密性。</li>
</ul>
<p>设置一览图
<img src="@source/NASdocs/maintain/img/user.png" alt="user.jpg" loading="lazy"></p>
<h2 id="四、共享设置" tabindex="-1"><a class="header-anchor" href="#四、共享设置" aria-hidden="true">#</a> 四、共享设置</h2>
<p>目前使用的共享设置有<code v-pre>SMB</code>共享与<code v-pre>webdav</code>共享</p>
</div></template>
