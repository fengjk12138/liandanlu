...
export default defineComponent({
    ...
    setup(props) {
        ...
        onMounted(() => {
            ...
            if (SUPPORT_PAGEVIEW)
                void import(
                /* webpackChunkName: "pageview" */ "vuepress-plugin-comment2/pageview").then(({ updatePageview }) => {
                    updatePageview();
                });
            watch(currentPage, () => {
        ...
        return () => h("div", { id: "article-list", class: "article-wrapper" }, currentArticles.value.length
            ? [
                ...currentArticles.value.map(({ info, path }, index) => h(DropTransition, { appear: true, delay: index * 0.04 }, () => h(ArticleItem, { key: path, info, path }))),
                h(Pagination, {
