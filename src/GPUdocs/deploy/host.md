...
```
该命令会列出当前系统可用安装的所有nvidia驱动版本，并提示推荐的驱动版本。
...
```bash
sudo apt install net-tools cifs-utils zfsutils-linux iotop sysstat nload libgl1 zip unzip nfs-common smartmontools pipx -o Acquire::http::proxy=$HTTPS_PROXY
```
安装显卡监控
```bash
pipx install nvitop
...
- 在远程服务器中添加trust，获取token
```bash
lxc config trust add
```

- 在本地服务器添加远程，并且上面获取的token
```bash
lxc remote add alpha 192.162.20.110
```
