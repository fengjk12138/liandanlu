...
<ul>
<li>配置静态ip。（<strong>使用新的端口映射方法不用分配静态ip了，不过为了分配端口映射时方便记忆，还是分配静态ip较为妥当</strong>）</li>
</ul>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div><p><code v-pre>attach</code>的eth0可以不用写</p>
<p>修改个人配置文件</p>
...
<li>
<p>为每个人分配GPU
在<code v-pre>device</code>下增加如下配置，按照<code v-pre>pci</code>的数值进行分配即可，</p>
<blockquote>
...
<div class="language-yaml line-numbers-mode" data-ext="yml"><pre v-pre class="language-yaml"><code>  <span class="token key atrule">gpu3</span><span class="token punctuation">:</span>
  ...
  <span class="token key atrule">gpu4</span><span class="token punctuation">:</span>
    <span class="token key atrule">id</span><span class="token punctuation">:</span> <span class="token number">4</span>
    <span class="token key atrule">type</span><span class="token punctuation">:</span> gpu
...
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code>lxc config device <span class="token function">add</span> mars-huanghansheng proxy-ssh proxy <span class="token assign-left variable">listen</span><span class="token operator">=</span>tcp:0.0.0.0:40xy0-40xy9 <span class="token assign-left variable">connect</span><span class="token operator">=</span>tcp:127.0.0.1:22,40xy1-40xy9
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>如果修改配置文件，加入以下内容</p>
<div class="language-yaml line-numbers-mode" data-ext="yml"><pre v-pre class="language-yaml"><code>  <span class="token key atrule">proxy-ssh</span><span class="token punctuation">:</span>
...
<p>GPU分配原则可以见<a href="https://docs.heartbreaker.icu/GPUdocs/user/fast.html#gpu%E5%88%86%E9%85%8D%E5%8E%9F%E5%88%99" target="_blank" rel="noopener noreferrer">其他文档<ExternalLinkIcon/></a></p>
</div>
