import{colors as $,fs as D}from"@vuepress/utils";import{Logger as W,entries as q,isAbsoluteUrl as O,isUrl as I,stripTags as z,getPageText as V,getAuthor as G,getDate as U,startsWith as K,checkVersion as Q}from"vuepress-shared/node";import{isString as P,isLinkHttp as X,removeEndingSlash as y,removeLeadingSlash as Y,isArray as Z,isFunction as tt}from"@vuepress/shared";const A="vuepress-plugin-seo2",s=new W(A),N=({lang:t,path:e,pathLocale:a},{pages:n,siteData:r})=>q(r.locales).map(([o,{lang:i}])=>({path:`${o}${e.replace(a,"")}`,lang:i})).filter(o=>P(o.lang)&&o.lang!==t&&n.some(({path:i})=>i===o.path)),C=({frontmatter:t},{options:{base:e}},{hostname:a})=>{const{banner:n,cover:r}=t;if(n){if(O(n))return f(a,e,n);if(I(n))return n}if(r){if(O(r))return f(a,e,r);if(I(r))return r}return null},H=({content:t},{options:{base:e}},{hostname:a})=>{const n=/!\[.*?\]\((.*?)\)/giu.exec(t);return n?n.map(([,r])=>O(r)?f(a,e,r):I(r)?r:null).filter(r=>r!==null):[]},f=(t,e,a)=>`${X(t)?y(t):`https://${y(t)}`}${e}${Y(a)}`,j=(t,e,a="",n="")=>{e in t&&(s.error(`"${$.magenta(e)}" is ${$.red("removed")}${n?`, please use ${$.magenta(n)} instead.`:" and no longer supported"}${a?`
